...
SCHBC(부천순천향) 와 API 구조가 완전 동일 — instcd, hsptlCode만 다름.
API:
  진료과 목록: POST /common/getCommDeptList.json (hsptlCode=seoul)
  의료진 목록: POST /seoul/doctr/list/selectIemList.json (deptNo)
  진료일정:   POST /seoul/doctr/home/selectEmrScheduleList.json (instcd=052, orddeptcd, orddrid, basedd)
"""
import logging
import httpx
...
INSTCD = "052"  # 서울=052 (부천=053, 천안=054, 구미=055)
HSPTL_CODE = "seoul"
DOCTR_KEY = "2947"  # 의료진 프로필 페이지 key (SCHBC와 동일 가정)

TIME_RANGES = {"morning": ("09:00", "12:00"), "afternoon": ("13:00", "17:00")}


class SchmcSeoulCrawler:
    """순천향대학교서울병원 크롤러"""

    def __init__(self):
        self.hospital_code = "SCHMC"
        self.hospital_name = "순천향대학교서울병원"
        self.headers = {
    ...
    async def _fetch_departments(self) -> list[dict]:
        ...
        async with httpx.AsyncClient(
            ...
            try:
                ...
                logger.info(f"[SCHMC] 진료과 {len(depts)}개")
                self._cached_depts = depts
                return depts
            except Exception as e:
                logger.error(f"[SCHMC] 진료과 목록 실패: {e}")
                self._cached_depts = []
                return []
    ...
    async def _fetch_dept_doctors(
        ...
        try:
            ...
            logger.error(f"[SCHMC] {dept_name} 의사 목록 실패: {e}")
            return []
        ...
        logger.info(f"[SCHMC] {dept_name}: {len(doctors)}명")
        return doctors
    ...
    async def _fetch_all(self) -> list[dict]:
        ...
        logger.info(f"[SCHMC] 총 {len(result)}명")
