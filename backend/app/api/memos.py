...
from app.models.database import Doctor, Hospital, MemoTemplate, VisitLog, VisitMemo
...
async def _load_memo(db: AsyncSession, memo_id: int) -> VisitMemo:
    query = (
        select(VisitMemo)
        .options(selectinload(VisitMemo.doctor).selectinload(Doctor.hospital))
        .where(VisitMemo.id == memo_id)
    )
...
async def _load_template(db: AsyncSession, template_id: Optional[int]) -> Optional[MemoTemplate]:
    if template_id is None:
        ...
        query = select(MemoTemplate).where(MemoTemplate.id == template_id)
...
async def create_memo(payload: VisitMemoCreate, db: AsyncSession = Depends(get_db)):
    memo = VisitMemo(
    ...
    memo = await _load_memo(db, memo.id)
...
async def list_memos(
    ...
    query = (
        select(VisitMemo)
        .options(selectinload(VisitMemo.doctor).selectinload(Doctor.hospital))
        .where(VisitMemo.user_id == DEFAULT_USER_ID)
    )
    if doctor_id:
...
async def update_memo(memo_id: int, payload: VisitMemoUpdate, db: AsyncSession = Depends(get_db)):
    memo = await _load_memo(db, memo_id)
...
async def summarize_memo(
    ...
    memo = await _load_memo(db, memo_id)
    template = await _load_template(db, payload.template_id or memo.template_id)
    fields: list[str] = []
...
async def list_memos_by_doctor(
    ...
    query = (
        select(VisitMemo)
        .options(selectinload(VisitMemo.doctor).selectinload(Doctor.hospital))
        .where(VisitMemo.doctor_id == doctor_id)
...
async def list_templates(db: AsyncSession = Depends(get_db)):
    query = (
        select(MemoTemplate)
        .where(MemoTemplate.user_id == DEFAULT_USER_ID)
...
async def create_template(payload: MemoTemplateCreate, db: AsyncSession = Depends(get_db)):
    t = MemoTemplate(
        user_id=DEFAULT_USER_ID,
        name=payload.name,
    ...
    if payload.is_default:
        await _unset_other_defaults(db)
    db.add(t)
    await db.commit()
    await db.refresh(t)
...
async def update_template(
    template_id: int, payload: MemoTemplateUpdate, db: AsyncSession = Depends(get_db)
):
    t = (await db.execute(select(MemoTemplate).where(MemoTemplate.id == template_id))).scalar_one_or_none()
    if not t:
    ...
    if data.get("is_default"):
        await _unset_other_defaults(db, exclude_id=t.id)
    for k, v in data.items():
        setattr(t, k, v)
    await db.commit()
    await db.refresh(t)
...
async def _unset_other_defaults(db: AsyncSession, exclude_id: Optional[int] = None):
    query = select(MemoTemplate).where(
        MemoTemplate.user_id == DEFAULT_USER_ID, MemoTemplate.is_default == True
    )
    rows = (await db.execute(query)).scalars().all()
    for row in rows:
